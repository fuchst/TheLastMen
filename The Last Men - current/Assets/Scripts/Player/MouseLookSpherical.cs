...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class MouseLookSpherical
    {
        ...
        public void LookRotation(Transform character, Transform camera)
        {
            ...
            yAngle += yRot;
            xAngle += xRot;
            m_helper.position = character.position;

            Vector3 localUp, localForward, localRight;
            localUp = m_helper.position.normalized;
            //localRight = Vector3.Cross (character.forward, localUp);
            //localForward = Vector3.Cross (localUp, localRight);
            //localForward = localForward.normalized;
            localForward = Vector3.Cross(-localUp, character.right);
            localForward = localForward.normalized;
            localRight = Vector3.Cross(localForward, -localUp);
            localRight = localRight.normalized;

            m_helper.rotation = Quaternion.LookRotation(localForward, localUp);
            m_helper.Rotate(Vector3.up, yRot, Space.Self);

            //Vector3 lastUp, lastForward, lastRight;
            //lastUp = m_helper.up;
            //lastForward = m_helper.forward;
            //lastRight = m_helper.right;

            //lastUp = character.position;
            //Vector3.OrthoNormalize (ref lastUp, ref lastForward, ref lastRight);
            //m_helper.rotation = Quaternion.LookRotation(lastForward, lastUp);

            //Vector3 newForward = Vector3.Cross(m_helper.right, character.position.normalized);
            //m_helper.rotation = Quaternion.LookRotation(newForward,character.position.normalized);
            //m_helper.LookAt(m_helper.position + newForward, character.position.normalized);
            ...
            //m_CameraTargetRot *= Quaternion.Euler (Mathf.Clamp(-xRot, MinimumX, MaximumX), 0f, 0f);
            //m_CameraTargetRot *= Quaternion.AngleAxis(-xRot, character.right);
            m_CameraTargetRot = Quaternion.identity;
            m_CameraTargetRot = m_CameraTargetRot * Quaternion.AngleAxis(-xAngle, Vector3.right);
            //m_CameraTargetRot = Quaternion.AngleAxis (-xAngle, m_helper.right);
